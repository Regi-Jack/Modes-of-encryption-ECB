...
int main() {
    ...
    unsigned char encrypted[1000 + AES_BLOCK_SIZE];  // Extra space for padding
    unsigned char decrypted[1000];  // Output for decrypted data
    
    // Prompt the user for input (up to 1000 bytes)
    printf("Enter the message (up to 1000 characters): ");
    ...
    // Decrypt the data
    aes_ecb_decrypt(encrypted, decrypted, key, SIZE_32, inputSize + (AES_BLOCK_SIZE - (inputSize % AES_BLOCK_SIZE)));
